...
def scrape():
    ...
    for i in range(1, 5):
        while True:
            ...
            # Check page number
            # create a new function that will take these hyperlinks one by one, get the
            # HTML and then we will scrape the data.
            # To make sure that we are scraping pages one by one, we’ll add a code in the
            # scrape() function to check the current page number
            # compare the current page number with counter i of the forloop
            # to save the value of the current page number we are using the get() function
            # click the buttons to go to the current page #
            # if the current page # is not the same as i, click the button by using
            # its XPATH of the respective buttons
...
# create a new empty list new_planets_data to save data from these new pages, and ask them to scrape the data like before.
new_planets_data = []

# in this function we are going to scrape the data from hyperlinks
...
def scrape_more_data(hyperlink):
    try:
        ...
        # create a variable 'page' and get the content of the HTML page using hyperlink
        # with get function of the requests module
        page = requests.get(hyperlink)
        # create a soup for parsing the HTML page
        soup = BeautifulSoup(page.content, "html.parser")

# create a new empty list called temp_list to store the new data temporarily
        temp_list = []
        # inspect the table and access the table row <tr><tr> and table data <td><td>
        # here we have to access all the table rows with the class name fact_row
        # in all the rows titles and values are present as <td><td> and we will be accessing these values only
        ...
        # after getting the data into temp_list, we will append it to the new_planets_data list
        # we are using except block for handling any errors or any exceptions as we did before to itterate and go to the next link
        new_planets_data.append(temp_list)
    except:
        time.sleep(1)
        #call scrape_more_data function and pass the hyperlink
        # it will scrape more data from the hyper link
...
with open("ppp.csv", "w") as f:
    ...
    csvwriter.writerows(new_planets_data)

# to check the data, print ten elements of new_planets_data
...
with open("planetdata_210_hyperlinks.csv", "w") as f:
    csvwriter = csv.writer(f)
    csvwriter.writerow(headers)
