...
browser = webdriver.Chrome("C:/Users/daran/Documents/PYTHON/PRO-C127/chromedriver")
...
# instead of just the index that a traditional loop gives.
# As we get the index (number of the element ) Using this index, if the index is 0 
# ( first element),
# Every element here contains the <a> tag or the anchor tag so we are first finding the 
# anchor tag inside the li_tag(list item), and then copying the inner HTML of it.
# Else, we want to directly copy the inner HTML of the li_tag.
# To facilitate this, in case a column is empty and we get an error, we are going to use
...
def scrape():
    ...
    for i in range(0, 210):
        ...
        for each_ul_tag in soup.find_all("ul", attrs={"class", "exoplanet"}):
            ...
            for index, each_li_tag in enumerate(li_tags):
                if index == 0:
                    ...
                    # for this we need to make sure that we treat the first li tag 
        ...
        # we want to go to the next stage after our code is done with all the ul tags <ul>
        # ...</ul> of the current page
    ...
    with open("planetdata_210.csv", "w") as f:
...
scrape()
# bs4 (BeautifulSoup) is a python module, which is famously used for parsing text as HTML 
# and then performing actions in it, such as
# finding specific HTML tags with a particular class/id, or listing out all the li tags
# inside the ul tags.
# Selenium, on the other hand, is used to interact with the webpage.
# It is famously used for automation testing, such as testing the functionality of a 
# website (Login/Logout/etc.)
